...
if (bsc_network == T){
  con <- connect_to_db()
  all_recv_deps <- DBI::dbGetQuery(con, 'select * from vwBase_receivers_deployments')
} else all_recv_deps <- all_deps() # grab the most recent local file


# get unique tag_deployID's from det_taghits_daily if specified
if (download_new_data == 1){
  if (bsc_network == F) stop('Not on BSC network')
  # TODO should also just test to see if the connection can be established before moving on
  # Get list of unique recv deployment Id'd for the site
  site_deploys <-
    all_recv_deps %>% filter(grepl(paste0('^', site, '$'), siteName),
                             grepl(proj, recvProjectID)) %>% 
    distinct(recvDeployID)
  query <- 
    paste0('select tag_deploy_id, [date], n_hits, a.tag_id, species_name, test
           from det_taghits_daily a
           left join tags_deployments b on b.id = a.tag_deploy_id
           where sensor_deploy_id in (',
           toString(site_deploys$recvDeployID), ')')
  daily_summary <- DBI::dbGetQuery(con, query)
  summary <- daily_summary %>% 
    filter(test != 1) %>% 
    group_by(tag_deploy_id) %>% 
    summarise(species_name,
              first_date = min(date),
              map_link = paste0('https://motus.org/data/track?tagDeploymentId=', tag_deploy_id))
} 


# download new data from R if specified
if (download_new_data == 1){
  motus_login()
